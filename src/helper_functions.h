...
inline double multivar_gauss_term(double x, double mu, double sig)
{
    return pow(x - mu, 2) / (2 * pow(sig, 2));
}

inline double multivar_gauss(double x, double y, double ux, double uy, double sigx, double sigy)
{
    double e = multivar_gauss_term(x, ux, sigx) + multivar_gauss_term(y, uy, sigy);
    e = exp(-e);
